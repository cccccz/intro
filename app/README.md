...
- **Outline**：点开后在 PDF 页上盖一层目录。有书签则点条目跳页；没有则按钮禁用（No outline）。再点 Outline、点页面空白或 Esc 关闭。
...
同一篇的不同挂接可以分别固定。偏好按资料库记住，重开这条侧注后仍生效；不会自动打开侧注，也不会改写笔记或 PDF。Close 和解除挂接依然收起整条子树。侧边自己的滚动和层级导航不受 Pin 限制。此功能尚未发布到桌面学习版，下一版封版时间待定。
