...
4. **划选一段**，点 **New side**：用 `addMark` 往宿主写入 `<<r id="…" to="…">>…<</r id="…">>`，并新建一篇侧边，作为下一列打开。宿主正文里该段保持高亮，并有导线连到打开的侧边列。
5. 在侧边里再划、再挂。列 = 深度。高亮与导线随滚动/窗口缩放更新。
6. **Hang existing…** 把选区挂到库里已有的一篇（复用）。点铆点条目则打开已挂的侧边。
7. **Close** 只从画面拿掉该列及其子树（导线随之隐藏）；磁盘上的铆点和篇还在，源段高亮仍在。
