...
需要官方 Codex CLI（验证版本 0.153.4）和 ChatGPT 登录。程序首先使用 `INTRO_CODEX_BIN` 指定的路径；未指定时自动查找 PATH 和 Windows Codex 安装目录（含版本子目录），验证版本后选择兼容的可执行文件，不要求从 Codex 开发环境启动。未登录时点击 **连接 Codex**，在浏览器完成官方登录后重新提问。没有复制或导出认证文件。
