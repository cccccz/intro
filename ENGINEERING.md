...
现阶段测标记代数（解析 / 嵌套 / 清除可逆 / 交叉视为损坏）、库文件读写、无 UI 的写回路（划选 → `addMark` → 建/开侧边 → 再挂）、PDF 侧车锚（QuadPoints 归一、overlay 增删、挂侧边不改 PDF 字节）、显示名 sidecar（`{id}.intro.meta.json` 读写，不改 `.intro.md` / PDF 字节）、以及视图像层的纯函数（rivet 范围 → 高亮片段；`rivetId →` 矩形列表的几何；列内 rendered HTML：先铆点再 markdown-it 子集 + 消毒 + 铆点占位；PDF 可见页窗口：相交页 ± 2 页 overscan；页码 clamp；fit-width 倍率 zoom clamp；分栏宽度 persist clamp）。Source 不跑 markdown。不做 UI snapshot，也不做 Electron E2E。
...
库是一个目录。文本篇 = `{id}.intro.md`。PDF 宿主 = `{id}.intro.host.json` + 不可变 `{id}.pdf` + `{id}.intro.overlay.json`。显示名（可选）= `{id}.intro.meta.json`（`{ title }`）；不是 YAML frontmatter，也不改标记 SoT。id → 路径由文件名主干解析。详见 [`app/README.md`](app/README.md)。
